...
describe("GET /content", () => {
  describe("when passed select data", () => {
    test("should respond with a 200 status code, body lenght=1", async () => {
      const response = await request(app)
        .get("/content")
        .set({ "Authorization": `Bearer 1` })
        .send({ data: { categoryid: 1 } });
      expect(response.statusCode).toBe(200);
      expect(response.body.data.length).toEqual(1);
      expect(response.body.data[0]).toHaveProperty("collection");
      // expect(response.body.data[0]).toHaveProperty("collection");
    });
  });
});
