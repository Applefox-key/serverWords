...
export const getAll = async (user) => {
  return await db_all(`SELECT * FROM collection_tags WHERE userid = ? ORDER BY name COLLATE NOCASE ASC`, [user.id]);
};

// Create a new tag
export const createTag = async (user, name) => {
  return await db_get(`INSERT INTO collection_tags (name, userid) VALUES (?, ?) RETURNING id`, [name, user.id]);
};

// Rename a tag
export const editTag = async (user, id, name) => {
  return await db_run(`UPDATE collection_tags SET name = ? WHERE id = ? AND userid = ?`, [name, id, user.id]);
};

// Delete a tag (collections_to_tags rows cascade automatically)
export const deleteTag = async (user, id) => {
  return await db_run(`DELETE FROM collection_tags WHERE id = ? AND userid = ?`, [id, user.id]);
};
...
export const setCollectionTags = async (collectionId, tagIds) => {
  ...
  await db_run(`DELETE FROM collections_to_tags WHERE collectionid = ?`, [collectionId]);
